...
- [hmacモジュール](#hmacモジュール)
  - [ビルドとテスト](#ビルドとテスト-1)
  - [使い方](#使い方-1)
  - [TLSの上にHMACによる完全性検証を重ねる理由](#tlsの上にhmacによる完全性検証を重ねる理由)
- [wolfSSLのビルド](#wolfsslのビルド)
  - [Step 0: autotoolsが必要](#step-0-autotoolsが必要)
  - [ビルド手順](#ビルド手順)
  - [.gitignore](#gitignore)
- [デモ用証明書](#デモ用証明書)
- [tlsモジュール(サーバー/クライアント)](#tlsモジュールサーバークライアント)
  - [ビルド](#ビルド)
  - [実行](#実行)
...
各モジュールを個別にビルドする代わりに、`src/run_all_tests.sh`で
まとめて実行できます(Linux/macOS両対応)。

```bash
./src/run_all_tests.sh
```

wolfSSLのビルド・証明書の生成は事前に済ませておく必要があります
(それぞれ「wolfSSLのビルド」「デモ用証明書」セクションを参照)。
...
ts_sha256_init(&ctx);
ts_sha256_update(&ctx, chunk1, chunk1_len);
ts_sha256_update(&ctx, chunk2, chunk2_len);   // 何回でも呼べる
ts_sha256_final(&ctx, digest);
```

手元にデータが全部揃っている場合は、`ts_sha256_buffer`で1回にまとめられます。

```c
uint8_t digest[TS_SHA256_DIGEST_SIZE];
ts_sha256_buffer(data, data_len, digest);
```
...
- `ts_sha256_transform`はブロック単体の圧縮処理、`ts_sha256_update`はそれを64バイトごとに呼び出すバッファリング層、という2層構造にしています
...
開発を通じて、実際に手を動かしたからこそ気づけた問題がいくつかありました。

- **wolfSSLとのシンボル衝突**：自作`sha256.h`の`SHA256_DIGEST_SIZE`が、wolfSSL自体の同名マクロと衝突。`ts_`プレフィックスで名前空間を分離して解決した
- **定数時間比較のタイポ**：`consttime_eq`内で`return diff == 0;`と書くべきところを`return diff = 0;`と記述しており、常に「改ざんあり」と誤判定するバグがあった。CIのログに鍵・ペイロード・タグの中身を出力するデバッグコードを一時的に仕込み、送受信データを1バイトずつ突き合わせることで原因を特定・修正した
